		...
		while (($donnee = oci_fetch_assoc($reqprep)) != false) {
			...
			$table = $table."<td>
					<form method='POST' action='gestionProduit.php?mode=U'>
						...
						<button type='submit'><img src='uploads/img/icon/crayon.png' height='32px'></button>
					</form>
				</td>";
			$table = $table."<td>
					<form method='POST' action='gestionProduit.php?mode=D'>
						<input type='hidden' name='nom' value='".$donnee["PNOM"]."'>
						<input type='hidden' name='idprod' value='".$donnee["IDPRODUIT"]."'>
						<button type='submit'><img src='uploads/img/icon/corbeille.png' height='32px'></button>
					</form>
				</td>";
			$table = $table."<td><a href=\"produit.php?nom=".$donnee["PNOM"]."\"><img src='uploads/img/icon/oeil.png' height='32px'></a></td></tr>";
		}
